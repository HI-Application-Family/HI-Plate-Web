...
const exportPDF = async () => {
  ...
  try {
    ...
    const imagesPromises = validMeals.map(async (item) => {
      ...
      const normalizedFilename = filename.replace('.webp', '.png');
      const imageUrl = imageMap[normalizedFilename as keyof typeof imageMap];
      
      if (!imageUrl) {
        console.warn(`No se encontró imagen para: ${normalizedFilename}`);
        return { 
